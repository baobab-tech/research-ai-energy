...
Agency reports, grid-operator data, regulatory filings, and corporate disclosures. Where the
defensible numbers are. Most are PDFs, which need extraction before they are readable.

## PDF extraction — required step
...
| Corporate | Scope 1/2/3, water withdrawal vs consumption, PPAs | company sustainability report PDFs |

## Reading grid-operator queue data

Interconnection requests are not capacity. The chain is:

    requested -> studied -> agreement signed -> under construction -> energised

Attrition is severe at each step and operators publish the stages separately. Always state which
stage a figure refers to. Historically under one in five queued US projects reaches operation.

## Reading corporate disclosures

- Find BOTH market-based and location-based Scope 2. The gap is the claim.
- Water: distinguish withdrawal from consumption. Reports often give only one.
- Check whether "renewable" is unbundled RECs, annual-matched PPAs, or hourly-matched supply.
- Note the reporting boundary: which subsidiaries, which regions, leased versus owned facilities.
- Check the restatement note. Prior-year figures are frequently revised without prominence.

## Superseded claims are evidence
